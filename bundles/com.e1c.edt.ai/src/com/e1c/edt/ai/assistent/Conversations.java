...
public class Conversations implements IConversations
{
        ...
        return Observables.create(observer -> {
            sessionService.getSessionAsync(projectId).whenComplete((session, error) -> {
                if (error == null)
                {
                    ...
                    {
                        ask(session.get(), conversationId, request, observer, cancellationToken);
                    }
                    else
                    {
    ...
    private void ask(Session session, String conversationId,
        ConversationAskRequest conversationRequest, IObserver<ConversationAskResponse> observer,
    ...
    private void processStream(Stream<String> stream, IObserver<ConversationAskResponse> observer,
        ICancellationToken cancellationToken)
    {
    ...
    public boolean process(String line, IObserver<ConversationAskResponse> observer,
        ICancellationToken cancellationToken)
    {
