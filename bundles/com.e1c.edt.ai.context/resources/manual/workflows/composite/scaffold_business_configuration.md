...
- For project creation, use the exact `create_configuration_project` workflow from JShellManual. Do not improvise a plain Eclipse project and then try to create `Configuration` through BM; `V8Project is null` means the project creation failed. The platform version is resolved by that workflow's Step 0 (explicit user version, or user's choice via `AskUser`, or the latest from `GetProjects` `available_platform_versions`).
